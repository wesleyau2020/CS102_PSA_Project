...
import com.psa.scheduler.PredictionQueryScheduler;
import com.psa.scheduler.VesselQueryScheduler;
...
public class ApiService {
    ...
    @Autowired
    VesselQueryScheduler vesselScheduler;

    @Autowired
    PredictionQueryScheduler predictionScheduler;
    /**
    ...
    public void updateApiSettings(ApiConfig apiConfig){
        if(apiConfig.getApiKey().trim().length() == 0) {
            apiConfig.setApiKey(this.apiConfig.getApiKey());
        }
        if(apiConfig.getVesselBerthingQueryInterval() != this.apiConfig.getVesselBerthingQueryInterval()) {
            updateVesselQueryInt(apiConfig.getVesselBerthingQueryInterval());
        }
        if(apiConfig.getPredictionQueryInterval() != this.apiConfig.getPredictionQueryInterval()) {
            updatePredictionQueryInt(apiConfig.getPredictionQueryInterval());
        }
    ...
    public void updateVesselQueryInt(int interval) {
        vesselScheduler.updateSchedule(interval);
    }
    
    public void updatePredictionQueryInt(int interval) {
        predictionScheduler.updateSchedule(interval);
    }
}
